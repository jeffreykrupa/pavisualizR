...
#' @param time_facet The time unit by which to facet the plot, options are
#'   'none', 'year', or 'month'. If 'month' faceting is done by both month and
#'   year.
#' @param point_size The size of every point on the plot.
#' @param point_alpha An integer between 0 and 1, (inclusive), indicating the
#'   opacity of each point on the plot.
...
sensor_hourlyavg_aqicolorsPlot <- function(
  ...
  data <- sensor %>%
    ...
    dplyr::mutate(
      ...
      aqi_category = cut(pm25, breaks = aqi$breaks_24,
                         labels = aqi$names)
    )
